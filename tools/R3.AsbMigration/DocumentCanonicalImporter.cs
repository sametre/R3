...
public sealed class DocumentCanonicalImporter
{
    ...
    public async Task<(int Sales, int Purchases, int InvoiceLines, int Movements)> ImportAsync()
    {
        ...
        using (var tx = c.BeginTransaction())
        {
            ...
            while (await r.ReadAsync())
            {
                ...
                var isSale = Int(r, 1) == 1;
                var documentTable = isSale ? "sales_documents" : "purchase_documents";
                var id = ResolveDocumentId(c, tx, documentTable, isSale ? "sales-document" : "purchase-document", legacy);
    ...
    private async Task<int> ImportMovementsAsync(Context context)
    {
        ...
        while (await r.ReadAsync())
        {
            ...
            using var ins = c.CreateCommand(); ins.Transaction = tx; ins.CommandText = "INSERT OR IGNORE INTO inventory_transactions(id,company_id,branch_id,warehouse_id,product_id,transaction_type,quantity,base_quantity,unit_id,document_type,reference_no,description,transaction_at,created_at,legacy_source,legacy_id) VALUES($id,$company,$branch,$warehouse,$product,$type,$qty,$qty,$unit,'LegacyStockMovement',$ref,$description,$at,$now,$source,$legacy)";
            var movementId = ResolveDocumentId(c, tx, "inventory_transactions", "stock-movement", legacy);
            Add(ins, "$id", movementId); Add(ins, "$company", _companyId); Add(ins, "$branch", context.Branch); Add(ins, "$warehouse", context.Warehouse); Add(ins, "$product", product); Add(ins, "$type", type); Add(ins, "$qty", qty); Add(ins, "$unit", unit); Add(ins, "$ref", Text(r, 1)); Add(ins, "$description", description); Add(ins, "$at", at); Add(ins, "$now", DateTime.UtcNow.ToString("O")); Add(ins, "$source", _sourceName); Add(ins, "$legacy", legacy); ins.ExecuteNonQuery();
            using (var mark = c.CreateCommand()) { mark.Transaction = tx; mark.CommandText = "UPDATE inventory_transactions SET legacy_source=$source,legacy_id=$legacy WHERE id=$id"; Add(mark, "$source", _sourceName); Add(mark, "$legacy", legacy); Add(mark, "$id", movementId); mark.ExecuteNonQuery(); }
            count++;
        }
        using (var rebuild = c.CreateCommand())
        {
            rebuild.Transaction = tx;
            rebuild.CommandText = @"INSERT INTO inventory_balances(company_id,branch_id,warehouse_id,product_id,quantity_on_hand,quantity_available,last_transaction_at,updated_at)
                SELECT $company,$branch,$warehouse,product_id,
                       SUM(CASE WHEN transaction_type='Inbound' THEN quantity ELSE -quantity END),
                       SUM(CASE WHEN transaction_type='Inbound' THEN quantity ELSE -quantity END),
                       MAX(transaction_at),$now
                FROM inventory_transactions
                WHERE legacy_source=$source AND warehouse_id=$warehouse AND variant_id IS NULL
                GROUP BY product_id";
            Add(rebuild, "$company", _companyId); Add(rebuild, "$branch", context.Branch); Add(rebuild, "$warehouse", context.Warehouse); Add(rebuild, "$source", _sourceName); Add(rebuild, "$now", DateTime.UtcNow.ToString("O")); rebuild.ExecuteNonQuery();
        }
    ...
    private string ResolveDocumentId(SqliteConnection c, SqliteTransaction tx, string table, string kind, long legacy)
    {
        using var find = c.CreateCommand(); find.Transaction = tx; find.CommandText = $"SELECT id FROM {table} WHERE legacy_source=$source AND legacy_id=$legacy LIMIT 1"; Add(find, "$source", _sourceName); Add(find, "$legacy", legacy);
        var id = find.ExecuteScalar()?.ToString();
        if (string.IsNullOrWhiteSpace(id))
        {
            id = StableId(kind, legacy.ToString());
            using var exists = c.CreateCommand(); exists.Transaction = tx; exists.CommandText = $"SELECT id FROM {table} WHERE id=$id LIMIT 1"; Add(exists, "$id", id); id = exists.ExecuteScalar()?.ToString() ?? StableId(kind, $"{_sourceName}:{legacy}");
        }
        using var mark = c.CreateCommand(); mark.Transaction = tx; mark.CommandText = $"UPDATE {table} SET legacy_source=$source,legacy_id=$legacy WHERE id=$id"; Add(mark, "$source", _sourceName); Add(mark, "$legacy", legacy); Add(mark, "$id", id); mark.ExecuteNonQuery();
        return id;
    }
    private static string ResolveLineId(SqliteConnection c, SqliteTransaction tx, string table, string oldValue, string newValue)
    {
        var oldId = StableId(table.StartsWith("sales", StringComparison.Ordinal) ? "sales-line" : "purchase-line", oldValue);
        using var q = c.CreateCommand(); q.Transaction = tx; q.CommandText = $"SELECT id FROM {table} WHERE id=$id LIMIT 1"; Add(q, "$id", oldId); return q.ExecuteScalar()?.ToString() ?? StableId(table.StartsWith("sales", StringComparison.Ordinal) ? "sales-line" : "purchase-line", newValue);
    }
