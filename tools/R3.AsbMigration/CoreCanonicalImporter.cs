    ...
    private async Task<int> ImportAccountsAsync()
    {
        ...
        while(await reader.ReadAsync())
        {
            ...
            using var up=c.CreateCommand(); up.Transaction=tx; up.CommandText="""
                ...
                ON CONFLICT(id) DO UPDATE SET code=$code,name=$name,account_type=$type,tax_office=$office,tax_number=$tax,phone=$phone,mobile_phone=$mobile,email=$email,is_active=$active,legacy_source=$source,legacy_id=$legacy,updated_at=$now
                """;
